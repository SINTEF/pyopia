...
class Pipeline():
    ...
    This common dictionary is therefore passed between steps so that data or variables generated by each step can be passed along the pipeline.
    The `steps` dict must contain keys: 'common' and 'classifier', which will run when Pipeline is initialised - the remaining steps will be run on Pipeline.run()
    the step called 'classifier' must return. (A more dynamic approach to key names will come in a future PR.)
