...
class Pipeline():
    ...
    The casses called in the Pipeline steps can be modified, and the names of the steps changed.
    New steps can be added or deleted as required.

    The classes called in the Pipeline steps need to take a dictionary and input and return a dictionary as output.
    This common dictionary is therefore passed between steps so that data
    or variables generated by each step can be passed along the pipeline.

    The `steps` dict must contain keys: 'common' and 'classifier',
    which will run when Pipeline is initialised - the remaining steps will be run on Pipeline.run()

    The step called 'classifier' must return a dict containing 'cl' in order to run successfully.

    (A more dynamic approach to key names in the common dictionary of data passed
    between steps will come in a future PR.)
